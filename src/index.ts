...
const initialize = (adDate: Date): NepaliDate => {
...
nepalidayjs.fromAd = (date: Date | string): NepaliDate => {
...
nepalidayjs.minDate = function (locale:Locale = "en") {
  const minDate = new NepaliDate({year:START_YEAR_BS,month:1,day:1,locale:locale});
  if(!minDate.isValid()){
...
nepalidayjs.maxDate = function (locale:Locale = "en") {
  const year = START_YEAR_BS+NEPALI_DATE_MAP.length-1;
...
nepalidayjs.isValid = function (date: NepaliDate | string) {
  if (date instanceof NepaliDate) {
    return date.isValid();
  } else if (typeof date === "string") {
